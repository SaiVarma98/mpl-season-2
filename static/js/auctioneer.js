...
async function bid() {
  ...
  try {
     ...
     * IMPORTANT:
     *
     * The bid endpoint in the current backend returns the full
     * auction result. We only update the parts that changed here
     * instead of immediately calling refresh(), which used to make
     * another 4 API requests after every bid.
     */
    ...
    if (result?.data) {
      ...
      // Update teams if backend supplied them.
      if (result.data.teams) {
        state.teams = result.data.teams;
      }

      // Update groups if backend supplied them.
      if (result.data.groups) {
        state.groups = result.data.groups;
      }

      // Update players if backend supplied them.
